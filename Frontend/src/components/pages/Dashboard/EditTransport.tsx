...
export default function EditTransportForm() {
  ...
  async function handleSubmit(
    ...
    try {
      ...
      if (response.status === 200) {
        showToast("Transport information updated successfully!", "success")
  ...
  const getTransportDetails = async () => {
    try {
      const response = await axios.get(`${baseURL}/v1/api/transport/id/${id}`)
      ...
      console.log("Transport data loaded:", formattedData)
    } catch (error) {
  ...
  return (
    <div className="flex bg-gray-100 justify-center items-center lg:px-8 min-h-screen px-2 py-1 sm:px-3">
      ...
      >
        <div>
          <h2 className="text-3xl text-center text-gray-900 font-extrabold mt-6">Edit Patient Transport</h2>
          <p className="text-center text-gray-600 text-sm mt-2">Please fill the details of your Patient Transport</p>
        </div>
