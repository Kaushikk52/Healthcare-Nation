...
export default function EditHomecareForm() {
  ...
  async function handleSubmit(
    ...
    try {
      ...
      const response = await axios.post(`${baseURL}/v1/api/homecare/edit`, {homecare: updatedValues, imagesToDelete : imagesToDelete}, {
        headers: { Authorization: `Bearer ${token}`, timeout: 20000 },
      })

      if (response.status === 200) {
        showToast("Homecare information updated successfully!", "success")
  ...
  const getHomecareDetails = async () => {
    try {
      const response = await axios.get(`${baseURL}/v1/api/homecare/id/${id}`)
      const data = response.data.homecare;
      ...
      console.log("Homecare data loaded:", formattedData)
    } catch (error) {
      console.error("Error fetching homecare details:", error)
      toast.error("Failed to load homecare data", {
        position: "bottom-right",
        duration: 3000,
      })
    }
  ...
  return (
    <div className="flex bg-gray-100 justify-center items-center lg:px-8 min-h-screen px-2 py-1 sm:px-3">
      ...
      >
        <div>
          <h2 className="text-3xl text-center text-gray-900 font-extrabold mt-6">Edit Homecare</h2>
          <p className="text-center text-gray-600 text-sm mt-2">Please fill the details of your Homecare</p>
        </div>
