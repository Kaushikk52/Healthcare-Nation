  ...
  const getStepFields = (stepNumber: number) => {
    ...
    async function handleSubmit(
      ...
      try {
        ...
        values.medicalFacilities = values.medicalFacilities?.map((facility) => ({id : facility}))
  
        const token = localStorage.getItem("token");
        const response = await axios.post(
          `${baseURL}/v1/api/orthotics/save`,
                ...
                {step === 4 && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <MultipleSelector
                        value={values.medicalFacilities?.map((b) => ({ label: b, value: b }))}
                        onChange={(newValue) => {
                          setFieldValue(
                            "medicalFacilities",
                            newValue.map((item) => item.value)
